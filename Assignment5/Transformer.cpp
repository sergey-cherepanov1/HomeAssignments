...
double Transformer::get_fuel() const
{
    return _fuel;
}
bool Transformer::get_form() const
{
    return _form;
}
unsigned Transformer::get_power() const
{
    return _core.get_power();
}
std::string Transformer::get_gun() const
{
    return _gun.get_gun();
}

void Transformer::change_gun(Gun new_gun)
{
    _gun.change_gun(new_gun);
}
void Transformer::reload(unsigned ammo)
{
    _ammo = ammo;
}
void Transformer::refuel(double fuel)
{
    _fuel = fuel;
}
...
std::istream& operator>>(std::istream& in, Transformer& transformer)
{
    ...
    std::cout << "Enter fuel: ";
    in >> fuel;
    std::cout << "Enter form (0 for vehicle, 1 for robot): ";
    in >> form;
    std::cout << "Enter gun type: ";
    in >> gun_type;
    transformer = Transformer(ammo, fuel, form, Gun(gun_type));
    return in;
}
